...
int kv_write(int kv_descriptor,uint32_t key,char* value,uint32_t value_length,int kv_overwrite){
	...
	mensagem=(char *)malloc((value_length+strlen(chave)+strlen(length)+5)*sizeof(char));
...
int kv_read(int kv_descriptor, uint32_t key,char* value, uint32_t value_length){
	...
	if((read(kv_descriptor,resposta,strlen(resposta)))<0){
		puts("falhou o read");
		free(mensagem);
		free(resposta);
		return -1;	
